...
describe "Pokemon" do
  ...
  describe "has caught all 151 from scraping" do
    ...
    it "knows Psyduck is the 54th pokemon" do
      expect(@db.execute("SELECT * FROM pokemon WHERE name = 'Psyduck';").flatten.first).to eq(54)
    end
  ...
  describe "saving more pokemon to the database" do
    ...
    it "knows the pokemon count increases" do
      expect(@db.execute("SELECT * FROM pokemon ORDER BY id DESC;").flatten.first).to eq(152)
    end
  ...
  describe "BONUS" do
    ...
    it "alters Pikachu's hp to 59" do
      pikachu.alter_hp(59)
      expect(@db.execute('SELECT hp FROM pokemon WHERE id = ?;', "#{pikachu.id}").flatten.first).to eq(59)
    end

    # Now we alter Magikarp's hp
    it "alters Magikarp's hp" do
      magikarp.alter_hp(0)
      expect(@db.execute('SELECT hp FROM pokemon WHERE id = ?;', "#{magikarp.id}").flatten.first).to eq(0)
    end
